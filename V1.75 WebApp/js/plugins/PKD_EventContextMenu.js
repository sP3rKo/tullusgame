...
/*:
 * @plugindesc (v.1.0)[PRO] Context Menu for Event
 ...
 */
/*:ru
 * @plugindesc (v.1.0)[PRO] Контекстное меню для события
 * @author Pheonix KageDesu
 * @target MZ MV
 * @url http://kdworkshop.net/plugins/event-context-menu
 *
...
// Generated by CoffeeScript 2.6.1
PKD_EventContextMenu.isPro = function() {
  return true;
};
...
(function() {
  ...
  //@[DEFINES]
  _ = ECM_FWindow_ContextMenu.prototype;
  _._createCustomElements = function() {
    this._createCommandsListWindow();
    this._createHeaderText();
  };
  _._createCommandsListWindow = function() {
    var h, playerCommandsListWindow, size, w;
    ({w, h} = this.params.listWindowSettings.size);
    size = new Rectangle(0, 0, w, h);
    playerCommandsListWindow = new ECM_Window_ContextOptionsList(size);
  ...
  _._createHeaderText = function() {
    this.headerText = new KDCore.UI.Sprite_UIText(this.params.headerTextSettings);
    return this.addChild(this.headerText);
  };
  _._updatePlacement = function() {
    if (this.params.isStaticPosition === true) {
      return this._updateStaticPlacement();
    } else {
      return this._updateDynamicPlacement();
    }
  };
  _._updateStaticPlacement = function() {
    this.x = this.params.position.x;
    this.y = this.param.position.y;
  };
  _._updateDynamicPlacement = function() {
    var h2, placementMargins, sector, w2, x, y;
    if (this.bindedChar == null) {
      return;
    }
    ...
    placementMargins = this.params.placementMargins;
    if (sector === 3 || sector === 1) {
      this.x = x + placementMargins.dxL;
    }
    if (sector === 2 || sector === 4) {
      this.x = x - this.params.size.w - placementMargins.dxR;
    }
    if (sector === 1 || sector === 2) {
      this.y = y + placementMargins.dyU;
    }
    if (sector === 3 || sector === 4) {
      this.y = y - this.params.size.h + placementMargins.dyD;
    }
  };
  _._updateTitle = function() {
    return this.headerText.draw(this.contextMenu.headerText);
  };
  _._storePositions = function() {
    this._myOldX = $gamePlayer.x;
    this._myOldY = $gamePlayer.y;
    if (this.isHaveCharacter()) {
      this._charOldX = this.bindedChar.x;
      this._charOldY = this.bindedChar.y;
    }
  };
  _._updateAutoCloseWhenMoving = function() {
    if (PKD_EventContextMenu.PP.isAutoCloseWhenPlayerMove()) {
      if ($gamePlayer.x !== this._myOldX || $gamePlayer.y !== this._myOldY) {
        PKD_ECMManager.terminate();
      }
    }
    if (this.isHaveCharacter() && PKD_EventContextMenu.PP.isAutoCloseWhenCharMove()) {
      if (this.bindedChar.x !== this._charOldX || this.bindedChar.y !== this._charOldY) {
        return PKD_ECMManager.terminate();
      }
    }
  };
  _._updateAutoCloseWhenMessage = function() {
  ...
  _._executeMenuItem = function(item) {
    ...
    try {
      ...
      this._processMenuItemActions(item);
      return this._processMenuItemStates(item);
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  };
  _._processMenuItemActions = function(item) {
    var cmId, e;
    try {
      if (String.any(item.action)) {
        try {
          eval(item.action);
        } catch (error) {
          e = error;
          KDCore.warning('ECM: Execute: Action', e);
        }
      }
      if (String.any(item.commonEvent)) {
        try {
          //cmId = KDCore.Utils.getEValue(item.commonEvent)
          cmId = parseInt(item.commonEvent);
          if (cmId > 0) {
            if (this.isHaveCharacter() && this.bindedChar instanceof Game_Event) {
              this.bindedChar.ecmStartCommonEventLocal(cmId);
            } else {
              $gameTemp.reserveCommonEvent(cmId);
            }
          }
        } catch (error) {
          e = error;
          KDCore.warning('ECM: Execute: Common Event', e);
        }
      }
      this._processMenuItemSelfSwitch(item.selfSwitchOn, true);
      return this._processMenuItemSelfSwitch(item.selfSwitchOff, false);
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  };
  _._processMenuItemSelfSwitch = function(switchString, isOn = true) {
    var e, key;
    try {
      if (!String.any(switchString)) {
        return;
      }
      if (switchString === "0") {
        return;
      }
      switchString = sOn.toUpperCase();
      if (!KDCore.SDK.checkSwitch(switchString)) {
        return;
      }
      if (!this.isHaveCharacter()) {
        return;
      }
      if (!(this.bindedChar instanceof Game_Event)) {
        return;
      }
      key = [$gameMap.mapId(), this.bindedChar.eventId(), switchString];
      return $gameSelfSwitches.setValue(key, isOn);
    } catch (error) {
      e = error;
      return KDCore.warning('ECM: Execute: SELF SWITCH', e);
    }
  };
  _._processMenuItemStates = function(item) {
    var e;
    if (item.startEvent === true) {
      try {
        if (this.isHaveCharacter() && this.bindedChar instanceof Game_Event) {
          this.bindedChar.start();
        }
      } catch (error) {
        e = error;
        KDCore.warning(e);
    ...
    if (this._isChainedMenuOpened(item)) { // * Exit from next processing
      return;
    }
    
    // * Последняя проверка опции
    if (item.closeMenu === true) {
      return PKD_ECMManager.terminate();
    } else {
      return this.sub().activate(); // * Always activate again if not closing
    }
  };
  
  //?VERSION
  _._isChainedMenuOpened = function(item) {
    return false;
  };
})();
...
(function() {
  ...
  //@[DEFINES]
  _ = Scene_Map.prototype;
  // * Берёт данные из GameTemp $gameTemp.ecmEventId и $gameTemp.ecmMenuId
  _.ecmShowContextMenu = function() {
    var character, settings;
    if (!PKD_ECMManager.isCanOpenNow()) {
      return;
    }
    if (!PKD_ECMManager.isValidCurrentState()) {
      return;
    }
    character = PKD_ECMManager.currentChar();
    if (character == null) {
      return;
    }
    settings = PKD_EventContextMenu.PP.getContextMenuWindowSettings();
    if (this.ecmIsContextMenuOpen()) {
      this.ecmCloseContextMenu();
    }
    this.ecmContextMenu = new ECM_FWindow_ContextMenu(this, settings.size.w, settings.size.h);
    // * Окно загружается ассинхронно, поэтому открыать по готовности
    this.ecmContextMenu.setOnReadyHandler(() => {
      return this.ecmContextMenu.openForCharacter(character);
    });
  };
  _.ecmCloseContextMenu = function() {
    if (!this.ecmIsContextMenuOpen()) {
      return;
    }
    this.ecmContextMenu.close();
    this.removeChild(this.ecmContextMenu);
    return this.ecmContextMenu = null;
  };
  _.ecmIsContextMenuOpen = function() {
    return (this.ecmContextMenu != null) && this.ecmContextMenu.isOpen();
  };
})();
...
(function() {
  ...
  //@[DEFINES]
  _ = Scene_Map.prototype;
  _.ecmOnMapCancelTouchComplete = function() {
    var beenClosed, e, eventUnderCursor;
    try {
      beenClosed = false;
      if (PKD_ECMManager.isMenuOpened()) {
        PKD_ECMManager.closeMenu();
        beenClosed = true;
      }
      eventUnderCursor = $gameMap.ecmGetEventUnderCursor();
      if (eventUnderCursor == null) {
        return beenClosed;
      }
      if (eventUnderCursor.ecmIsHaveValidContextMenu()) {
        if (PKD_EventContextMenu.PP.isPlayClickEffect()) {
          PKD_ECMManager.playClickEffect();
        }
        if (PKD_ECMManager.isCanOpenNow()) {
          PKD_ECMManager.openMenuForEvent(eventUnderCursor);
          return true; // * не открывать игровое меню в этом случае
        } else {
          return PKD_EventContextMenu.PP.isNotOpenGameMenuOnRClickOnEvent();
        }
      } else {
        // * Опция, что при нажатии правой кнопкой мышки по событию (даже без меню) не показывать игрвое меню
        return PKD_EventContextMenu.PP.isNotOpenGameMenuOnRClickOnEvent();
      }
    } catch (error) {
      e = error;
...
(function() {
  ...
  //?VERSION
  _._isChainedMenuOpened = function(item) {
    if (String.any(item.chainMenu) && PKD_ECMManager.isValidContextMenuId(item.chainMenu)) {
      console.log("OPEN CHAIN MENU");
      PKD_ECMManager.closeMenu();
      setTimeout((function() {
        return PKD_ECMManager.openContextMenu(item.chainMenu, $gameTemp.ecmEventId);
      }), 1);
      return true;
    }
    return false;
...
(function() {
  ...
  //?VERSION
  _._prepareCommandsForCurrentChar = function() {
    var data, i, len, option, ref;
    this._distance = this._getDistanceFromPlayerToBindedCharacter();
    this._items = [];
    data = PKD_ECMManager.getContextMenuData(this.menuId);
    if (data == null) {
      return;
    }
    ref = data.options;
