...
/*:
 * @plugindesc (v.1.0)[PRO] Simple Achievements
...
@command LockAchievement
@text Lock achievement
...
@param unlockAchievementME
@text Unlock Achievement Sound Effect
@type file
@dir audio/me
@require 1
@default
...
@param unlockPreviewImagePosition
@text Unlock Preview Image Position
@type select
@option None
@value none
@option Center
...
@value bottomRight
@default none
@desc The position of the unlock preview image.




@param unlockPreviewAnimation:int
@text Unlock Preview Animation
@type animation
@default 0
@desc The animation played when the achievement is unlocked.
...
@default
@require 1
@desc An additional image for the achievement.
...
@param unlockAchievementME
@text Звук разблокировки достижения
@type file
@dir audio/me
@require 1
@default
...
@param unlockPreviewImagePosition
@text Позиция изображения предварительного просмотра разблокировки
@type select
@option None
@value none
@option Center
...
@value bottomRight
@default none
@desc Позиция изображения предварительного просмотра разблокировки.




@param unlockPreviewAnimation:int
@text Анимация предварительного просмотра разблокировки
@type animation
@default 0
@desc Анимация, воспроизводимая при разблокировке достижения.




@param extraImage
@text Дополнительное изображение
@type file
@dir img/PKD_SimpleAchievements
@default
@require 1
@desc Дополнительное изображение для достижения.
...
const UnlockAchievement = (id) => {
    ...
    try {
        if (!AchievementsStorage.isAchievementUnlocked(id)) {
            AchievementPresentation.onAchievementUnlocked(id);
        }
...
const LockAchievement = (id) => {
    // Logic to lock the achievement
    try {
        AchievementsStorage.setAchievementUnlocked(id, false);
        if (PP.getIsStoreAchievementsGlobal()) {
            ConfigManager.save();
        }
    }
...
(function (PP) {
    ...
    */
    function LoadPluginSettings() {
        _loader = new KDX.ParamLoader("PKD_SimpleAchievements");
        PKD_SimpleAchievements_PluginCommands.RegisterPluginCommands();
    }
    ...
    */
    ...
    // * If TRUE, achievements are stored globally across all games
    // * If FALSE, achievements are stored per game (save file)
    function getIsStoreAchievementsGlobal() {
        return getLoaderParam('isStoreAchievementsGlobal', true);
    }
    PP.getIsStoreAchievementsGlobal = getIsStoreAchievementsGlobal;
    function getHiddenAchievementName() {
        return getLoaderParam('hiddenAchievementName', "???");
    }
    PP.getHiddenAchievementName = getHiddenAchievementName;
    function getHiddenAchievementDescription() {
        return getLoaderParam('hiddenAchievementDescription', "");
    }
    PP.getHiddenAchievementDescription = getHiddenAchievementDescription;
    function achievementsMenuOptionTitle() {
        return getLoaderParam('achievementsMenuOptionTitle', "Achievements");
    }
    PP.achievementsMenuOptionTitle = achievementsMenuOptionTitle;
    function isAddAchievementsMenuOption() {
        return getLoaderParam('isAddAchievementsMenuOption', true);
    }
    PP.isAddAchievementsMenuOption = isAddAchievementsMenuOption;
    function isAddAchievementsTitleOption() {
        return getLoaderParam('isAddAchievementsTitleOption', true);
    }
    PP.isAddAchievementsTitleOption = isAddAchievementsTitleOption;
    function getOpenMenuSE() {
        return getLoaderParam('openMenuSE', "");
    }
    PP.getOpenMenuSE = getOpenMenuSE;
    function getMenuBGM() {
        return getLoaderParam('menuBGM', "");
    }
    PP.getMenuBGM = getMenuBGM;
    let _achievementsCache = null;
    function getAchievements() {
        if (PKD_SimpleAchievements['isPro']) {
            // For test purposes, we return the same achievements twice
            return getLoaderParam('achievements', []);
        }
        else {
            if (_achievementsCache) {
                return _achievementsCache;
            }
            let achievements = getLoaderParam('achievements', []);
            if (achievements.length > 10) {
                console.warn("PKD_SimpleAchievements: Pro version required to access more than 10 achievements.");
                window.alert("PKD_SimpleAchievements: Pro version required to access more than 10 achievements.");
            }
            // Return first 10 achievements if not Pro version
            _achievementsCache = achievements.slice(0, 10);
            return _achievementsCache;
        }
    }
    PP.getAchievements = getAchievements;
})(PP || (PP = {}));
...
(function (PKD_SimpleAchievements_PluginCommands) {
    ...
    function RegisterPluginCommands() {
        ...
        registerCommand_LockAchievement();
    ...
    function registerCommand_LockAchievement() {
        PluginManager.registerCommand(PluginName, "LockAchievement", function (args) {
            try {
                const parameters = (KDX.ParamParser.ParseParameters(args, PluginName));
                if (KString.any(parameters.achievementId)) {
                    LockAchievement(parameters.achievementId);
                }
            }
            catch (error) {
                console.warn(error);
...
PKD_SimpleAchievements['isPro'] = true;




var AchievementPresentation;
(function (AchievementPresentation) {
    function onAchievementUnlocked(id) {
        try {
            var achievementData = PP.getAchievements().find(a => a.id === id);
            if (!achievementData) {
                console.warn(`Achievement with ID ${id} not found.`);
                return;
            }
            try {
                if (achievementData.unlockAchievementME && achievementData.unlockAchievementME !== "") {
                    AudioManager.playMe({ name: achievementData.unlockAchievementME, volume: 100, pitch: 100, pan: 0, pos: 0 });
                }
            }
            catch (error) {
                console.warn(error);
            ...
            try {
                if (achievementData.unlockPreviewAnimation && achievementData.unlockPreviewAnimation > 0) {
                    if (KDX.isMV()) {
                        $gamePlayer.requestAnimation(achievementData.unlockPreviewAnimation);
                    }
                    else {
                        //@ts-ignore
                        $gameTemp.requestAnimation([$gamePlayer], achievementData.unlockPreviewAnimation);
                    }
                }
            }
            catch (error) {
            ...
            try {
                Sprite_AchievementPreview.Show(achievementData);
            }
            catch (error) {
                console.warn(error);
            }
    ...
    AchievementPresentation.onAchievementUnlocked = onAchievementUnlocked;
})(AchievementPresentation || (AchievementPresentation = {}));


//$[ENCODE]
var AchievementsStorage;
(function (AchievementsStorage) {
    let _achievementsData = {};
    let _newlyUnlockedAchievements = [];
    function isAchievementUnlocked(achievementId) {
        return _achievementsData[achievementId] === true;
    }
    AchievementsStorage.isAchievementUnlocked = isAchievementUnlocked;
    function isAchievementNew(achievementId) {
        return isAchievementUnlocked(achievementId) && _newlyUnlockedAchievements.includes(achievementId);
    }
    AchievementsStorage.isAchievementNew = isAchievementNew;
    function markAchievementAsViewed(achievementId) {
        if (isAchievementNew(achievementId)) {
            _newlyUnlockedAchievements = _newlyUnlockedAchievements.filter(id => id !== achievementId);
        }
    }
    AchievementsStorage.markAchievementAsViewed = markAchievementAsViewed;
    function setAchievementUnlocked(achievementId, isUnlocked) {
        _achievementsData[achievementId] = isUnlocked;
        if (!isAchievementNew(achievementId)) {
            _newlyUnlockedAchievements.push(achievementId);
        }
    }
    AchievementsStorage.setAchievementUnlocked = setAchievementUnlocked;
    function getAllAchievements() {
        return Object.assign({}, _achievementsData);
    }
    AchievementsStorage.getAllAchievements = getAllAchievements;
    function getNewAchievements() {
        return [..._newlyUnlockedAchievements];
    }
    AchievementsStorage.getNewAchievements = getNewAchievements;
    function setNewAchievements(achievements) {
        _newlyUnlockedAchievements = achievements ? [...achievements] : [];
    }
    AchievementsStorage.setNewAchievements = setNewAchievements;
    function setAllAchievements(achievements) {
        _achievementsData = Object.assign({}, achievements);
    }
    AchievementsStorage.setAllAchievements = setAllAchievements;
    function resetAchievements() {
        _achievementsData = {};
        _newlyUnlockedAchievements = [];
    }
    AchievementsStorage.resetAchievements = resetAchievements;
    function unlockedAchievementsCount() {
        return Object.values(_achievementsData).filter(unlocked => unlocked).length;
    }
    AchievementsStorage.unlockedAchievementsCount = unlockedAchievementsCount;
    ...
    _.makeData = function () {
        ...
        try {
            if (PP.getIsStoreAchievementsGlobal()) {
                ...
                config['pNewAchievements'] = AchievementsStorage.getNewAchievements();
            }
        }
        catch (error) {
            console.warn(error);
    ...
    _.applyData = function (config) {
        ...
        try {
            if (PP.getIsStoreAchievementsGlobal()) {
                ...
                AchievementsStorage.setNewAchievements(config['pNewAchievements']);
            }
        }
        catch (error) {
            console.warn(error);
...
(() => {
    ...
    pkdRegisterLocalNUIFile("NUI_AchievementPreviewImage");
    pkdRegisterLocalNUIFile("NUI_AchievementExtraImage");
    pkdRegisterLocalNUIFile("NUI_AchievementsProgressText");
    pkdRegisterLocalNUIFile("NUI_AchievementNewBadge");
    //@[ALIAS]
...
(() => {
    ...
    _.pluginCommand = function (command, args) {
        ...
        try {
            ...
            if (command === "LockAchievement" && args && args.length > 0 && KString.any(args[0])) {
                LockAchievement(args[0]);
                return;
            }
    ...
    _.onBeforeSave = function (...args) {
        ...
        try {
            if (!PP.getIsStoreAchievementsGlobal()) {
                ...
                this['pNewAchievements'] = AchievementsStorage.getNewAchievements();
            }
        }
        catch (error) {
            console.warn(error);
    ...
    _.onAfterLoad = function (...args) {
        ...
        try {
            if (!PP.getIsStoreAchievementsGlobal()) {
                ...
                AchievementsStorage.setNewAchievements(this['pNewAchievements']);
            }
        }
        catch (error) {
            console.warn(error);
...
class Scene_AchievementsMenu extends Scene_MenuBase {
    constructor() {
        ...
        this._isBGMStarted = false;
        this._progressTextElement = null;
    }
    needsCancelButton() {
        return false;
    }
    ...
    create() {
        ...
        try {
            KAudio.PlaySE(PP.getOpenMenuSE());
        }
        catch (error) {
            console.warn(error);
        }
        try {
            this._progressTextElement = new Sprite_AchievementsProgressText();
            this.addChild(this._progressTextElement);
        }
        catch (error) {
            console.warn(error);
        }
    ...
    unlockedAmount() {
        return AchievementsStorage.unlockedAchievementsCount();
    }
    isMorePagesAvailable() {
        return this._currentPageIndex < this.maxPage() - 1;
    }
    isNextPageButtonVisible() {
        return this.isMorePagesAvailable();
    }
    ...
    currentPageIndex() {
        return this._currentPageIndex + 1; // +1 to make it 1-based index
    }
    achievementsAmount() {
        return PP.getAchievements().length;
    }
    achievementsPerPage() {
        return this._achievementsMenuSprite.uiConstant("achievementsPerPage") || 4; // Default to 4 if not defined
    }
    maxPage() {
        return Math.ceil(this.achievementsAmount() / this.achievementsPerPage());
    }
    createAchievementsWindow() {
        var _a, _b, _c, _d;
        KDNUI.FromScheme(this._scheme(), this, null, 0);
        try {
            (_a = this._achievementsMenuSprite) === null || _a === void 0 ? void 0 : _a.refreshBindings(this, true);
            (_b = this._closeButton) === null || _b === void 0 ? void 0 : _b.addClickHandler(this._onCloseButtonClick.bind(this));
            (_c = this._nextPageButton) === null || _c === void 0 ? void 0 : _c.addClickHandler(this._onNextPageButtonClick.bind(this));
            (_d = this._previousPageButton) === null || _d === void 0 ? void 0 : _d.addClickHandler(this._onPreviousPageButtonClick.bind(this));
            setTimeout(() => {
                this._fillAchievementsPageContainer();
            }, 50); // Delay to ensure the scene is fully initialized
        }
        catch (error) {
            console.warn(error);
        }
    ...
    _fillAchievementsPageContainer() {
        var _a, _b, _c;
        try {
            this._pageContainer.removeChildren();
            const sprites = this._getAchievementsSpritesPerPage();
            sprites.forEach(sprite => {
                this._pageContainer.addChild(sprite);
                let position = this._achievementsMenuSprite.uiConstant("achievementItemPosition");
                sprite.refreshBindings(sprite, true);
                sprite.setPosition(position.x, position.y);
                sprite.refreshComponents();
            });
            (_a = this._counterText) === null || _a === void 0 ? void 0 : _a.refreshBindings(this, true);
            (_b = this._previousPageButton) === null || _b === void 0 ? void 0 : _b.refreshBindings(this, true);
            (_c = this._nextPageButton) === null || _c === void 0 ? void 0 : _c.refreshBindings(this, true);
        }
        catch (error) {
            console.warn(error);
        }
    }
    _getAchievementsSpritesPerPage() {
        const achievements = PP.getAchievements();
        const startIndex = this._currentPageIndex * this.achievementsPerPage();
        const endIndex = startIndex + this.achievementsPerPage();
        return achievements.slice(startIndex, endIndex).map(achievement => {
            return new Sprite_AchievementItem(achievement);
        });
    }
    _scheme() {
        return PKD_SimpleAchievements.GetNUIFile("NUI_AchievementsMenu");
    }
}
...
class Sprite_AchievementExtraImage extends KNSpriteWithScheme {
    constructor(_achievement, _parentSprite) {
        super();
        this._achievement = _achievement;
        this._parentSprite = _parentSprite;
        var parentSprite = this._parentSprite;
        if (this.parentSpriteId() != "") {
            parentSprite = this.tryFindParentSpriteById(this.parentSpriteId());
        }
        if (parentSprite) {
    ...
    _scheme() {
        return PKD_SimpleAchievements.GetNUIFile("NUI_AchievementExtraImage");
    }
    parentSpriteId() {
        try {
            return this.uiConstant('childOf') || "";
        }
        catch (error) {
            console.warn(error);
        }
    ...
    extraImageName() {
        try {
            if (!this._achievement) {
                return ""; // Fallback image if something goes wrong
            }
            return this._achievement.extraImage || ""; // Default to "" if no extra image is set
        }
        catch (error) {
            console.warn(error);
        }
    ...
    tryFindParentSpriteById(id) {
        try {
            // Try to find any sprite (recursively) of this._parentSprite with field ['id'] == id
            if (this._parentSprite) {
                return this.findChildById(this._parentSprite, id) || this._parentSprite;
            }
        }
        catch (error) {
            console.warn(error);
...
class Sprite_AchievementItem extends KNSpriteWithScheme {
    constructor(_achievement) {
        ...
        this._newBadgeImage = null;
        this._extraImage = new Sprite_AchievementExtraImage(this._achievement, this);
        this._newBadgeImage = new Sprite_AchievementNewBadge(this._achievement, this, AchievementsStorage.isAchievementNew(this._achievement.id));
        this.refreshBindings(this, true);
        this.refreshComponents();
        AchievementsStorage.markAchievementAsViewed(this._achievement.id);
    }
    destroy(options) {
        try {
            //console.log("Destroying Sprite_AchievementItem for achievement:", this._achievement.id);
            AchievementsStorage.markAchievementAsViewed(this._achievement.id);
        }
        catch (error) {
            console.warn(error);
    ...
    _refreshComponents() {
        try {
            if (!this._extraImage)
                return;
            this._extraImage.refreshBindings(this._extraImage, true);
            //@ts-ignore
            var jsonScheme = this._extraImage.children[0].getJsonSchema();
            if (!jsonScheme)
                return;
            this._extraImage.setPosition(jsonScheme.bindings.position.x, jsonScheme.bindings.position.y);
            if (!this._newBadgeImage)
                return;
            this._newBadgeImage.refreshBindings(this._newBadgeImage, true);
            //@ts-ignore
            var jsonScheme = this._newBadgeImage.children[0].getJsonSchema();
            if (!jsonScheme)
                return;
            this._newBadgeImage.setPosition(jsonScheme.bindings.position.x, jsonScheme.bindings.position.y);
        }
        catch (error) {
            console.warn(error);
        }
...
class Sprite_AchievementNewBadge extends KNSpriteWithScheme {
    constructor(_achievement, _parentSprite, _isNew) {
        super();
        this._achievement = _achievement;
        this._parentSprite = _parentSprite;
        this._isNew = _isNew;
        if (this._parentSprite) {
            this._parentSprite.addChild(this);
        }
        this.refreshBindings(this, true);
    }
    _scheme() {
        return PKD_SimpleAchievements.GetNUIFile("NUI_AchievementNewBadge");
    }
    isAchievementNew() {
        try {
            return this._isNew;
            //if(!this._achievement) return false;
            //return AchievementsStorage.isAchievementNew(this._achievement.id);
        }
        catch (error) {
            console.warn(error);
        }
...
class Sprite_AchievementPreview extends KNSpriteWithScheme {
    static Show(achievement) {
        if (!achievement.unlockPreviewImagePosition) {
            return; // Exit if no position is defined
        }
        if (achievement.unlockPreviewImagePosition == "none") {
            return; // Exit if the position is set to "none"
        }
        const sprite = new Sprite_AchievementPreview(achievement);
    ...
    constructor(_achievement) {
        ...
        this._achievement = _achievement;
        this.refreshBindings(this, true);
        this._stayTimer = new KDX.TimedUpdate(this.stayTimeInSeconds() * 60, () => {
            this.visible = false;
            this.removeFromParent();
        });
        this._stayTimer.once();
    }
    update() {
        super.update();
        this._stayTimer.update();
    }
    positionX() {
        try {
            if (!this._achievement) {
                return "0"; // Fallback position if something goes wrong
            }
            if (this._achievement.unlockPreviewImagePosition) {
                var value = this._achievement.unlockPreviewImagePosition;
                switch (value) {
                    case "center":
                        return "center";
                    case "topLeft":
    ...
    stayTimeInSeconds() {
        try {
            return this.uiConstant("stayTimeInSeconds") || 3; // Default to 3 seconds if not set
        }
        catch (error) {
            console.warn(error);
        }
        return 3; // Default to 3 seconds if something goes wrong
    }
    achievementImage() {
        try {
            if (!this._achievement) {
    ...
    _scheme() {
        return PKD_SimpleAchievements.GetNUIFile("NUI_AchievementPreviewImage");
    }
}


class Sprite_AchievementsProgressText extends KNSpriteWithScheme {
    _scheme() {
        return PKD_SimpleAchievements.GetNUIFile("NUI_AchievementsProgressText");
    }
    progressTextColor() {
        try {
            if (this.unlockedAmount() >= this.achievementsAmount()) {
                return this.uiConstant("completedTextColor") || "#f7e300"; // Default to yellow if not set
            }
            return this.uiConstant("defaultTextColor") || "#5d3c11"; // Default to brown if not set
        }
        catch (error) {
            console.warn(error);
        }
